...
class Fringes:
    ...
    _Dmax: int = 2
    _amax: float = 2
    _gmax: float = 3  # most screens have a gamma value of ~2.2
    ...
    def _modulate(self, frames: int | Sequence[int]) -> np.ndarray:
            ...
            for i in range(self.K):
                ...
                for n in range(self._N[d, i]):  # todo: allow any n: for n in self.n[d, i]:
                    if frame in frames:
                        ...
                        val = self.Imax * (self.E * (1 + self.V * np.cos(k * x_ - w * t - self.p0))) ** self.g
    ...
    def _demodulate(
        ...
        # todo: spu here
        # if self.uwr == "spatial":
        #     # todo: which sets to choose for spu()?
        #     x = spu(p)
        #     # todo: r = ...
    ...
    def _multiplex(self, I: np.ndarray) -> np.ndarray:
        ...
        if self.SDM:
            assert not self.FDM
            assert self.grid in self._choices["grid"][:2]
            assert self.D == 2
    ...
    def encode(self, frames: int | Sequence[int] | None = None) -> np.ndarray:
        ...
        if frames is None:
            ...
            frames = np.array(frames, int).ravel() % self.T  # t=0:T:1
    ...
    def decode(
        ...
        if self._ambiguous:
            ...
            x = spu(x, uwr_func)
            x *= self._l[:, 0, None, None, None] / PI2
    ...
    def uncertainty(
        ...
        ux = up / PI2 * l  # positional uncertainties
    ...
    def grid(self, grid: str):
        ...
        if (self.SDM or self.uwr == "FTM") and self.grid not in self._choices["grid"][:2]:
            logger.error(f"Couldn't set 'grid': grid not in {self._choices["grid"][:2]}'.")
            return
    ...
    def SDM(self, SDM: bool):
        ...
        if _SDM:
            ...
            if self.grid not in self._choices["grid"][:2]:
                _SDM = False
                logger.error(f"Couldn't set 'SDM': grid not in {self._choices["grid"][:2]}'.")
    ...
    @property
    def K(self) -> int:  # todo: | Sequence[int]:
    ...
    def p0(self, p0: float):
        _p0 = float(np.sign(p0) * np.abs(p0) % PI2)
    ...
    def uwr(self) -> str:
        ...
        if self.K == 1 and np.all(self._N == 1) and self.grid in self._choices["grid"][:2]:
            # todo: v >> 1, i.e. l ~ 8
        ...
        for d in range(self.D):
            ...
            if lcm > 0:
                for i in range(self.K):
                    ...
                    # if np.any(crt[d, :i] % self._m[d, i] == 1):  # one element already equals 1 modulo m[i]
            ...
            for i in range(self.K):
                for j in range(i + 1, self.K):
                    if np.gcd(self._m[d, i], self._m[d, j]) != 1:
                        j0 = j
            if j0 > 0:
                crt[d, j0] = 0
    ...
    #     return self.up / PI2 * self._l  # local positional uncertainties
    #
    # @property
